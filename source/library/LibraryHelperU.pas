...
type
  TEnviromentVariable = class(TLZObject)
  private
    FName: string;
    FValue: string;
...
type
  TEnvironmentVariables = class(TLZObject)
  private
...
type
  TDelphiInstallation = class(TLZObject)
  private
    FRegistryKey: string;
    ...
    function GetUserRoamingFolder: string;
    procedure SaveEnvOptions;
    function GetEnvOptionsFileName: TFileName;
  public
    constructor Create(ARegistryKey: string);
    destructor Destroy; override;
    procedure Load;
    procedure Save(ADeduplicate, AUpdateEnvOptions: boolean);
    procedure Cleanup;
    ...
    property StudioVersion: integer read GetStudioVersion;
    ...
    property EnvOptionsFileName: TFileName read GetEnvOptionsFileName;
  end;
...
type
  TLibraryHelper = class(TLZObject)
  private
...
uses
  ...
  Winapi.ShlObj, System.IniFiles, Lazy.Log, System.StrUtils, LibraryEnvOptionsU;
...
function TDelphiInstallation.GetEnvOptionsFileName: TFileName;
begin
  Result := GetBDSUserRoamingFolder + 'EnvOptions.proj';
end;

function TDelphiInstallation.GetUserRoamingFolder: string;
begin
  Result := IncludeTrailingPathDelimiter(GetShellFolderPath(CSIDL_APPDATA));
end;
...
function TDelphiInstallation.GetBDSUserRoamingFolder: string;
begin
  Result := GetUserRoamingFolder;
  Result := IncludeTrailingPathDelimiter(Result + 'Embarcadero\BDS');
  Result := IncludeTrailingPathDelimiter
    (Result + IntToStr(GetStudioVersion) + '.0');
...
begin
  ...
  if FileExists(LFolder) then
  begin
    LFolder := ExtractFilePath(LFolder);
  end;
...
begin
  ...
  case LProductVersion of
    37:
      Result := 'Delphi Florence';
    29:
      Result := 'Delphi Athens';
    28:
...
var
  LPath: string;
begin
  LPath := ExcludeTrailingPathDelimiter(FRegistryKey);
...
procedure TDelphiInstallation.Save(ADeduplicate, AUpdateEnvOptions: boolean);
begin
  if ADeduplicate then
    Deduplicate;
  SaveEnvironmentVariables;
  SaveLibraries;
  if AUpdateEnvOptions then
  begin
    SaveEnvOptions;
  end
  else
...
procedure TDelphiInstallation.SaveEnvOptions;
var
  LEnvOptions: TEnvOptions;
begin
  LEnvOptions := TEnvOptions.Create;
  try
    LEnvOptions.EnvOptionsFileName := EnvOptionsFileName;
    LEnvOptions.Load;

    LEnvOptions.ApplyLibraryPaths('Win32', FLibraryWin32);
    LEnvOptions.ApplyLibraryPaths('Win64', FLibraryWin64);
    LEnvOptions.ApplyLibraryPaths('Android', FLibraryAndroid32);
    LEnvOptions.ApplyLibraryPaths('Android64', FLibraryAndroid64);
    LEnvOptions.ApplyLibraryPaths('iOSDevice64', FLibraryIOS64);
    LEnvOptions.ApplyLibraryPaths('OSX64', FLibraryOSX64);
    LEnvOptions.ApplyLibraryPaths('OSXARM64', FLibraryOSXARM64);
    LEnvOptions.ApplyLibraryPaths('Linux64', FLibraryLinux64);

    LEnvOptions.Save;
    Log(LEnvOptions.LogText);
  finally
    FreeAndNil(LEnvOptions);
  end;
end;
