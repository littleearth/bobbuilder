...
uses Classes, Windows, SysUtils, Graphics, Vcl.Imaging.pngimage,
  Vcl.Imaging.jpeg, Lazy.Utils, Lazy.Types, Bob.Common;
...
type
  ...
  protected
    ...
    function ProcessMarkdownLine(
      const ALine: string;
      var AOutput: string;
      var AInCodeBlock: Boolean): Boolean;
  ...
  public
    function Convert(
      AInputFileName: TFileName;
      AOutputFileName: TFileName): Boolean;
  end;
  ...
  protected
    ...
    function CheckCommandParameter(
      var AMessage: string;
      var AInputFile: TFileName;
      var AOutputFile: TFileName): Boolean;
  public
...
begin
  ...
  LMultiByteLen := MultiByteToWideChar(CP_UTF8, 0, PAnsiChar(AUTF8Text),
    Length(AUTF8Text), PChar(Result), Length(Result));
  SetLength(Result, LMultiByteLen);
end;
...
begin
  ...
  begin
    ...
    if (AText[i] = '\') and (i < Length(AText)) and
      CharInSet(AText[i + 1], ['a' .. 'z', 'A' .. 'Z', '''', '-']) then
    begin
      // This is the start of an RTF code - don't escape the backslash
      Result := Result + AText[i];
      LInRTFCode := True;
    end
    else if LInRTFCode and not CharInSet(AText[i],
      ['a' .. 'z', 'A' .. 'Z', '0' .. '9', '''', '-']) then
    begin
    ...
    begin
      ...
      case AText[i] of
        ...
        #13:
          Result := Result + '\par'#13#10' ';
        #10:
          ; // Ignore, we handle line breaks explicitly
      else
        Result := Result + AText[i];
      end;
    end;
...
begin
  ...
  Result := '\f1\fs22 ' + Result + '\f0\fs24 ';
  // Consolas font (f1), smaller size
end;
...
function TMarkdownToRTF.ProcessHeading(
  const ALine: string;
  ALevel: Integer): string;
const
  SizeMap: array [1 .. 6] of Integer = (36, 28, 22, 18, 16, 14);
begin
  Result := '\b\fs' + IntToStr(SizeMap[Min(ALevel, 6)]) + ' ' + ALine +
    '\b0\fs24\par'#13#10;
end;

function TMarkdownToRTF.ProcessList(
  const AText: string;
  ALevel: Integer): string;
begin
...
function TMarkdownToRTF.ProcessLink(
  const AText: string;
  const AUrl: string): string;
var
  LDispRaw: string;
...
begin
  ...
  if (Length(LDispRaw) >= 2) and (LDispRaw[1] = '`') and
    (LDispRaw[Length(LDispRaw)] = '`') then
    LDispRtf := ProcessCodeInline(Copy(LDispRaw, 2, Length(LDispRaw) - 2))
  else if (Pos('\', LDispRaw) > 0) or (Pos('{', LDispRaw) > 0) or
    (Pos('}', LDispRaw) > 0) then
  ...
  Result := '{\field{\*\fldinst HYPERLINK "' + LUrl + '"}{\fldrslt{\ul\cf2 ' +
    LDispRtf + '}}} ';
end;

function TMarkdownToRTF.ProcessImage(
  const AAltText: string;
  const AUrl: string): string;
var
  ...
  try
    ...
    begin
      if ExtractFilePath(FInputFileName) <> '' then
        LImagePath := IncludeTrailingPathDelimiter
          (ExtractFilePath(FInputFileName)) + LImagePath;
    end;
    ...
    if not((LExtension = '.png') or (LExtension = '.jpg') or
    ...
    Result := '{\pict\' + LFormat + '\picw' + IntToStr(LOriginalWidthTwips) +
      '\pich' + IntToStr(LOriginalHeightTwips) + '\picwgoal' +
      IntToStr(LDisplayWidthTwips) + '\pichgoal' + IntToStr(LDisplayHeightTwips)
      + ' ' + LHexString + '}';
  except
...
begin
  ...
  Result := ' ' + '{\field{\*\fldinst HYPERLINK "' + LUrl +
    '"}{\fldrslt{\ul\cf2 ' + LText + '}}} ';
end;

function TMarkdownToRTF.ProcessMarkdownLine(
  const ALine: string;
  var AOutput: string;
  var AInCodeBlock: Boolean): Boolean;
var
  ...
  begin
    ...
    begin
      ...
      begin
        LField := ProcessLink(TempLink.Groups[1].Value,
          ExtractUrlFromLinkParam(TempLink.Groups[2].Value));
        // inject bold start after fldrslt{
        LField := StringReplace(LField, '{\fldrslt{\ul\cf2 ',
          '{\fldrslt{\b \ul\cf2 ', []);
    ...
    begin
      ...
      begin
        Text := StringReplace(Text, TempImage.Value,
          ProcessImage(TempImage.Groups[1].Value,
          TempImage.Groups[2].Value), []);
    ...
    begin
      ...
      begin
        Text := StringReplace(Text, TempLink.Value,
          ProcessLink(TempLink.Groups[1].Value,
          ExtractUrlFromLinkParam(TempLink.Groups[2].Value)), []);
        if Pos('{\field{', Text) > 0 then
          Break;
        TempLink := TempLink.NextMatch;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Length(LMatchedUrl) >= 2) and (LMatchedUrl[1] = '<') and
            (LMatchedUrl[Length(LMatchedUrl)] = '>') then
            LMatchedUrl := Copy(LMatchedUrl, 2, Length(LMatchedUrl) - 2);
          Text := StringReplace(Text, TempUrl.Value,
            ProcessUrl(LMatchedUrl), []);
        end
        else
        begin
    ...
    begin
      ...
      begin
        Text := StringReplace(Text, TempCode.Value,
          ProcessCodeInline(TempCode.Groups[1].Value), []);
      ...
      begin
        Text := StringReplace(Text, TempBold.Value,
          ProcessBold(TempBold.Groups[1].Value), []);
      ...
      begin
        Text := StringReplace(Text, TempItalic.Value,
          ProcessItalic(TempItalic.Groups[1].Value), []);
...
begin
  ...
  LIsTableLine := (Pos('|', ALine) > 0) and
    (TRegEx.IsMatch(ALine, '^\s*\|.*\|\s*$') or TRegEx.IsMatch(ALine,
    '^\s*[^\n]*\|[^\n]*$'));
  // Detect separator row (---, :---:, etc.) optionally bounded by pipes
  LIsSeparator := TRegEx.IsMatch(Trim(ALine),
    '^\|?\s*[:\-]+\s*(\|\s*[:\-]+\s*)+\|?$');
  ...
  begin
    ...
    if (Length(LLineNoPipes) > 0) and (LLineNoPipes[Length(LLineNoPipes)] = '|')
    then
    ...
    try
      ...
      begin
        ...
        AOutput := AOutput + '\clbrdrt\brdrs\brdrw10' + '\clbrdrl\brdrs\brdrw10'
          + '\clbrdrb\brdrs\brdrw10' + '\clbrdrr\brdrs\brdrw10' + '\cellx' +
          IntToStr(LColRight);
      end;
      ...
      begin
        if LIsHeaderRow then
          AOutput := AOutput + '\intbl \f1\b ' + RTFEscape(LCells[i]) +
            ' \b0\f0\cell'
        else
          AOutput := AOutput + '\intbl \f1 ' + RTFEscape(LCells[i]) +
            ' \f0\cell';
      end;
  ...
  begin
    ...
    AOutput := AOutput + ProcessHeading(LProcessedLine,
      LHeadingMatch.Groups[1].Length);
    Exit;
  end;
  ...
  begin
    ...
    if (Pos('{\field{', LProcessedLine) > 0) or
      (Pos('\pict\', LProcessedLine) > 0) or (Pos('\b', LProcessedLine) > 0) or
      (Pos('\i', LProcessedLine) > 0) or (Pos('\f1', LProcessedLine) > 0) or
      (Pos('\cf', LProcessedLine) > 0) or (Pos('\ul', LProcessedLine) > 0) then
    begin
...
function TMarkdownToRTF.Convert(
  AInputFileName: TFileName;
  AOutputFileName: TFileName): Boolean;
var
  LInput: TStringList;
...
begin
  ...
  try
    ...
    LFileStream := TFileStream.Create(AInputFileName, fmOpenRead or
      fmShareDenyWrite);
    try
    ...
    LRTF := LRTF +
      '{\fonttbl{\f0\fswiss\fcharset0 Aptos;}{\f1\fmodern\fcharset0 Consolas;}{\f2\fswiss\fcharset0 Arial;}{\f3\ftech\fcharset0 Symbol;}}'#13#10;
...
function TMarkdownToRTFConsole.CheckCommandParameter(
  var AMessage: string;
  var AInputFile: TFileName;
  var AOutputFile: TFileName): Boolean;
var
...
begin
  ...
  begin
    ...
    if Result then
      AMessage := Format('Successfully converted %s to %s',
        [LInputFile, LOutputFile])
    else
